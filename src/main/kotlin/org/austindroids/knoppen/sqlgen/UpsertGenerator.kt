...
class UpsertGenerator(
    ...
    data class GenerationResult(
        ...
        data class NamedStatement(
            val table: String,
            /** Row(s) covered by this statement — a single index for one-row-per-statement mode, a wider range for a multi-row batch. */
        ...
        fun prettyPrint(): String = buildString {
            ...
            if (sql.isNotEmpty()) {
                appendLine()
                appendLine(render(sql, fileStats))
            }
        }

        fun toSqlString(): String = render(sql, fileStats)

        companion object {
            /** Renders a YAML front-matter header plus every statement — shared by whole-result and per-output-file rendering. */
            fun render(statements: List<NamedStatement>, fileStats: List<FileStats>): String = buildString {
                appendLine(header(statements, fileStats))
                statements.forEach { stmt ->
                    appendLine()
                    val rowLabel = if (stmt.rowRange.first == stmt.rowRange.last)
                        "row[${stmt.rowRange.first}]"
                    else
                        "rows[${stmt.rowRange.first}-${stmt.rowRange.last}]"
                    appendLine("-- Table: ${stmt.table}, $rowLabel")
                    appendLine(stmt.sql)
                }
            }

            /** Left-justifies a front-matter "key:" to a fixed column so values line up. */
            private fun field(key: String, value: String): String =
                "-- " + "$key:".padEnd(14) + value

            private fun header(statements: List<NamedStatement>, fileStats: List<FileStats>): String {
                val now     = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
                val user    = System.getProperty("user.name", "unknown")
                val appName = ResourceConstants.APP_NAME
                val version = ResourceConstants.APP_VERSION

                val fileStatsByTable = fileStats.groupBy { it.tableName }

                val lines = mutableListOf<String>()
                lines += "-- ---"
                lines += field("title", "Generated by $appName version $version")
                lines += field("user", user)
                lines += field("created", now)
                lines += field("statements", "${statements.size} statement(s)")
                lines += "--"
                statements.groupBy { it.table }.keys.forEach { table ->
                    lines += "-- $table:"
                    fileStatsByTable[table].orEmpty().forEach { fs ->
                        lines += "--   ${fs.filePath}:"
                        lines += "--     lines: ${fs.lines}"
                        lines += "--     checksum: ${fs.checksum}"
                    }
                }
                lines += "-- ---"
    ...
    fun generateAll(
        ...
        val statements = if (!generateSql || allErrors.any { it.severity == DataValidationError.Severity.ERROR }) {
            ...
            val (builtStatements, buildErrors) = buildAllStatements(sortedTables, allRows)
            allErrors.addAll(buildErrors)
            if (buildErrors.any { it.severity == DataValidationError.Severity.ERROR }) emptyList() else builtStatements
        }
        ...
        val outputFiles = if (statements.isEmpty()) {
            ...
            buildOutputFileGroups(sortedTables, statements, allFileStats, baseOutputDir)
        }
    ...
    /** Counts physical lines in [content], regardless of a trailing newline. */
    ...
    /** CRC32 checksum of [content]'s raw bytes — cheap, deterministic, no extra dependency. */
    private fun checksumOf(content: String): Long =
        CRC32().apply { update(content.toByteArray()) }.value
    ...
    private fun buildOutputFileGroups(
        ...
        return tablesByPath.map { (path, tables) ->
            ...
            val groupedFileStats  = allFileStats.filter { it.tableName in tableNames }
            GenerationResult.OutputFileGroup(
                path   = path,
                tables = tables.map { it.tableName },
                sql    = GenerationResult.render(groupedStatements, groupedFileStats)
            )
        }
    }
     ...
     * pre-loaded, pre-validated rows. Returns statements alongside any errors
     * discovered while batching (e.g. a multi-row batch whose rows don't share
     * the same insertable columns) — callers must treat ERROR-severity entries
     * the same way as any other validation error.
     */
    private fun buildAllStatements(
        sortedTables: List<TableSchema>,
        allRows: Map<String, List<JsonNode>>
    ): Pair<List<GenerationResult.NamedStatement>, List<DataValidationError>> {
        ...
        for (tableSchema in sortedTables) {
            ...
            // ── Resolve every row's DataRow first — generator/default resolution
            // must stay strictly per-row and in order regardless of how the
            // resulting rows are later grouped into statements below.
            val resolvedRows = rows.mapIndexed { rowIndex, rowNode ->
            ...
            if (multiRowBatchSize <= 1) {
                ...
                val (batchStatements, batchErrors) = buildBatchedStatements(tableSchema, resolvedRows)
                statements.addAll(batchStatements)
                errors.addAll(batchErrors)
            }
        }

        return statements to errors
    }

    /**
     * Groups [resolvedRows] into multi-row batches (see [chunkRows]) and generates
     * one [SqlDialect.generateMultiRowUpsert] statement per batch. A batch whose
     * rows don't all share the same insertable columns is reported as an error and
     * skipped rather than generating ambiguous SQL.
     */
    private fun buildBatchedStatements(
        tableSchema: TableSchema,
        resolvedRows: List<DataRow>
    ): Pair<List<GenerationResult.NamedStatement>, List<DataValidationError>> {
        val statements = mutableListOf<GenerationResult.NamedStatement>()
        val errors     = mutableListOf<DataValidationError>()

        for (chunk in chunkRows(tableSchema, resolvedRows)) {
            val mismatchedColumn = findColumnMismatch(tableSchema, chunk.rows)
            if (mismatchedColumn != null) {
                errors.add(DataValidationError(
                    table    = tableSchema.tableName,
                    rowIndex = chunk.range.first,
                    field    = mismatchedColumn,
                    line     = null,
                    message  = "Rows ${chunk.range} for table '${tableSchema.tableName}' disagree on whether " +
                            "column '$mismatchedColumn' is present. Multi-row batching requires every row in " +
                            "a batch to share the same insertable columns — add a schema default for " +
                            "'$mismatchedColumn' or supply an explicit value in every row."
                ))
                continue
            }
            val sql = dialect.generateMultiRowUpsert(chunk.rows)
            statements.add(GenerationResult.NamedStatement(tableSchema.tableName, chunk.range, sql))
        }
     ...
     * in the batch: PostgreSQL rejects a multi-row `INSERT ... ON CONFLICT DO
     * UPDATE` if two rows in the *same* statement target the same conflict key
     * ("ON CONFLICT DO UPDATE command cannot affect row a second time"), even
    ...
    private fun chunkRows(tableSchema: TableSchema, resolvedRows: List<DataRow>): List<RowChunk> {
        ...
        fun flush() {
            ...
            val indices = currentChunk.map { it.index }
            chunks.add(RowChunk(indices.first()..indices.last(), currentChunk.map { it.value }))
            currentChunk = mutableListOf()
            seenKeys = mutableSetOf()
        }
    ...
    /** Joins a row's values for the ON CONFLICT target columns into a comparable key. */
    private fun conflictKeyOf(row: DataRow, target: List<String>): String =
        target.joinToString(",") { colName -> row.fields[colName]?.toString() ?: "null" }

    /**
     * Returns the name of a no-default column whose presence differs across
     * [rows], or null if every row agrees. Columns with a schema default are
     * always included in a row's insertable-column set regardless of the row's
     * own data, so they can never disagree across rows — only a column with no
     * default (present in some rows' data files but not others) can.
     */
    private fun findColumnMismatch(tableSchema: TableSchema, rows: List<DataRow>): String? {
        if (rows.size <= 1) return null
        val noDefaultColumns = tableSchema.columns.filter { it.default == null }
        for (col in noDefaultColumns) {
            val presence = rows.map { it.fields.containsKey(col.name) }
            if (presence.distinct().size > 1) return col.name
        }
        return null
    }
